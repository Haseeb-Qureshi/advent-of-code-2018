...
def set_up_indexes(size_multiplier = 1)
  @target = [10, 10] # reversed i, j
  x, y = @target
...
heap = FakeHeap.new([[0, 0, TORCH, 0]])
seen = Set.new
best_distances = Hash.new { |h, k| h[k] = FLOAT::INFINITY }
until heap.empty?
  i, j, equipment, dist = heap.pop_min
  next if seen.include?([i, j, equipment])
  if [j, i] == @target
    puts "FOUND IT!"

    puts equipment == TORCH ? dist : dist + 7
    break
  end
  ...
  neighbors(i, j).each do |i2, j2|
    ...
    candidates = options.reject do |new_equipment|
      seen.include?([i2, j2, new_equipment])
    ...
    candidates.each { |cand| heap << cand }
  end
...
until heap.empty?
  ...
  neighbors(i, j).each do |i2, j2|
    ...
    candidates.each do |cand|
      heap << cand
      i2, j2, new_equipment, dist = cand

      if dist < best_distances[[i2, j2, new_equipment]][1]
        best_distances[[i2, j2, new_equipment]] = [[i, j, equipment], dist]
      end
    end
  end
